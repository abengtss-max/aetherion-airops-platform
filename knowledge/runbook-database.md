...
> Operational runbook for the Aetherion AirOps platform. PostgreSQL Flexible
> Server `aetherion-pg-*` (database `aetherion`) and Azure Managed Redis
...
3. Recycle the affected pods to release held connections:
   `kubectl rollout restart deploy/crew-scheduling -n aetherion`.
